                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="text-[10px] uppercase tracking-widest text-slate-400 bg-slate-950/70 border-b border-white/5">
                            ...
                            <th class="px-6 py-4">Lucky Number</th>
                        ...
                        <?php foreach ($ledgerEntries as $row): ?>
                            <tr class="hover:bg-white/[0.01] transition-colors">
                                ...
                                <td class="px-6 py-4 font-mono font-black">
                                    <?php if (!empty($row['is_released']) && !empty($row['lucky_number'])): ?>
                                        <div class="flex gap-1 tracking-widest">
                                            <?php foreach (str_split($row['lucky_number']) as $lDigit): ?>
                                                <span class="inline-block bg-emerald-950/80 border border-emerald-500/30 text-emerald-400 px-2 py-0.5 rounded text-xs"><?php echo htmlspecialchars($lDigit, ENT_QUOTES, 'UTF-8'); ?></span>
                                            <?php endforeach; ?>
                                        </div>
                                    <?php else: ?>
                                        <span class="text-[10px] uppercase tracking-wider text-slate-500 font-sans">Pending Draw</span>
                                    <?php endif; ?>
                                </td>
                    ...
                    <div class="p-4 space-y-3 bg-slate-900/40 hover:bg-slate-900/60 transition-colors">
                        ...
                        <div class="space-y-2">
                            <div class="flex justify-between items-center">
                                <span class="text-[9px] font-bold uppercase text-slate-500 tracking-wider">Your Sequence</span>
                            ...
                            <div class="flex justify-between items-center">
                                <span class="text-[9px] font-bold uppercase text-slate-500 tracking-wider">Lucky Number</span>
                                <div>
                                    <?php if (!empty($row['is_released']) && !empty($row['lucky_number'])): ?>
                                        <div class="flex gap-1 font-mono font-black tracking-widest">
                                            <?php foreach (str_split($row['lucky_number']) as $lDigit): ?>
                                                <span class="inline-block bg-emerald-950/80 border border-emerald-500/30 text-emerald-400 px-2 py-0.5 rounded text-xs"><?php echo htmlspecialchars($lDigit, ENT_QUOTES, 'UTF-8'); ?></span>
                                            <?php endforeach; ?>
                                        </div>
                                    <?php else: ?>
                                        <span class="text-[10px] font-mono text-slate-500">Pending</span>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-2 pt-2 border-t border-white/[0.03] text-[10px] font-mono text-slate-500">
                            <div>
                                <span class="block uppercase text-[8px] tracking-wider text-slate-600 font-bold">Allocation Weight</span>
                                <span class="text-slate-200 font-black">₦<?php echo number_format($row['amount'], 2); ?></span>
                            </div>
