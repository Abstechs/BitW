...
function requestWithdrawal($user_id, $amount, $method = 'wallet', $payout_details = '') {
    global $pdo;
    ...
    try {
        ...
        // Explicitly include payout_details column mapping in the SQL array payload
        $stmt = $pdo->prepare("INSERT INTO withdrawals (user_id, amount, method, payout_details, status, created_at) VALUES (?, ?, ?, ?, 'pending', NOW())");
        $stmt->execute([$user_id, $amount, $method, $payout_details]);
